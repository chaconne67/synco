...
### 섹션별 독립 추출 — 반드시 따라야 할 핵심 규칙

같은 회사·학교가 여러 섹션에 나오면 **반드시 각각 별도 항목으로** 만들고
source_section에 출처를 명시하세요. **하나의 항목으로 통합하면 안 됩니다.**

흔한 위반 사례 (절대 하지 말 것):
- "SUMMARY OF WORK EXPERIENCE"와 "DETAILED WORK EXPERIENCE" 두 섹션에
  같은 삼성전자 정보가 있을 때 한 섹션만 추출하는 것
- 한국어 경력란과 영문 경력란에 같은 회사가 있을 때 한쪽만 추출하는 것
- 자기소개서에서 회사명이 다시 언급되는데 추출에서 빠뜨리는 것

올바른 처리 (반드시 따를 것):
입력에 같은 삼성전자가 다음 3곳에 있다면:
  [한국어 경력란] 삼성전자 / 차장 / 2018.03 ~ 2021.06
  [영문 경력란]   Samsung Electronics / Manager / 2016.03 ~ 2021.06
  [자기소개서]   "삼성전자 부장으로 2018-2022 재직"
출력은 careers 배열에 3개 항목:
  {"company": "삼성전자", ..., "source_section": "한국어 경력란"}
  {"company": "Samsung Electronics", ..., "source_section": "영문 경력란"}
  {"company": "삼성전자", ..., "source_section": "자기소개서"}

이유: 정규화 시스템이 섹션 간 날짜·직책을 비교해야 위조를 탐지합니다.
하나로 통합해서 보내면 차이가 영원히 사라지고 다음 단계에서 복구 불가능합니다.

### 회사명 표기 — company vs company_en

한 항목 안에서 한국어 표기와 영문 표기가 함께 적혀 있는 경우만 한 항목에 둘 다 채우세요.
예) "㈜코스모앤컴퍼니 (COSMO & Company, Inc.)"
  → {"company": "㈜코스모앤컴퍼니", "company_en": "COSMO & Company, Inc."}

다른 섹션에 있는 영문 표기는 위 "섹션별 독립 추출" 규칙에 따라 별도 항목으로 만드세요.
영문 표기만 있고 한국어 표기가 없으면 company에 영문을 넣고 company_en은 null 또는 같은 값.
...
### placeholder 금지 — 미상 회사·학교 처리 원칙

회사명·학교명·날짜를 본문에서 정확히 식별할 수 없으면 careers/educations
항목을 만들지 마세요. 다음 placeholder는 금지합니다:
- company/institution: "미상", "unknown", "?", "N/A", "회사명 미상", "Company A" 등
- start_date/end_date: "미상", "?", 2000-01 같은 임의값

이유: 가짜 placeholder는 후보자 검색·매칭에서 노이즈가 되며,
검수자에게 잘못된 신뢰를 줍니다. 정보 부재는 fields를 null/빈 배열로 두는
것이 정직합니다.

**회사명만 알고 시작일을 모르면 careers 금지** — careers 항목은 최소
{company, start_date} 둘 다 본문에서 식별되어야 만들 수 있습니다. 회사명만
있고 입사일 정보가 없으면 careers가 아니라 career_etc[]에 다음 형태로
보존하세요:
- {"type": "기간 미상 경력", "company": "선익", "description": "약 6-7년 근무"}

예외: 본문에 "현재 재직 중" 같은 명시적 재직 표시가 있으면 start_date 없이도
careers 항목 생성 가능 (start_date는 null로 두고 is_current=true).

대안 — 정보를 잃지 마세요:
- 직무 내용·기술 활동만 있고 회사 식별 불가 → career_etc[]에 description으로 보존
  (예: {"type": "직무 활동", "description": "Power Builder ERP 개발, DB 관리, 서버 관리"})
- 학위/연구 정보만 있고 학교 식별 불가 → education_etc[]에 보존
- 논문·학회 발표·특허·제안서 채택 등 연구 활동 → career_etc[]에 별도 항목으로 보존
- 인턴·계약직·단기 활동도 본인 경력이고 날짜가 명확하면 careers에 정상 항목
  으로 보존하세요 (career_etc로 옮기지 말 것). 정규직 vs 비정규직 구분은
  position·duration이 표현합니다.

### 이력서가 아닌 보조 문서 처리 — 경력기술서·포트폴리오·연구활동서

이력서 본체와 별도로 제출되는 보조 문서(경력기술서, 포트폴리오, 연구활동서,
논문 목록 등)가 단독으로 들어올 수 있습니다. 이 경우:
- 본인 회사 경력 라인(YYYY.MM~YYYY.MM 회사명 직책)이 전혀 없으면 careers는 빈 배열
- 학교/학위 라인이 명확하지 않으면 educations는 빈 배열
- 직무·연구·논문·학회·제안서 등 모든 정보는 해당 카테고리의 etc[]에 보존
- 이런 문서도 후보자 정보로서 보존 가치가 있습니다 (연구직 채용 등). 제외하지 마세요.
...
educations에는 **대학교, 대학원, 전문대학(2-3년제 college)만 포함**하세요.
고등학교, 중학교, 초등학교는 educations에 넣지 마세요.

이유: 채용 매칭은 대학 학위 이상을 기준으로 하며, 고등학교 정보는
검색·필터링에 사용되지 않습니다. educations 배열에 고등학교가 섞이면
"대졸 우대" 같은 단순 카운트 필터가 부정확해집니다.

예외 — 다음 경우만 고등학교를 education_etc[]에 별도 보관 (educations 아님):
- 후보자가 대학을 다니지 않았고 고등학교가 최종 학력인 경우
- 외국 학제(예: 미국 prep school)에서 고등학교가 진학·진로에 의미가 큰 경우

확실하지 않으면 educations에 넣지 마세요.
...
### "같은 회사"의 정의 — 법인 단위 분리 원칙

"같은 회사"는 **같은 법인**을 의미합니다. 다음은 통합하지 말고 별도 항목으로 유지하세요:
- 자회사·관계사: IBM Korea ≠ IBM Singapore ≠ IBM Korea Technical Solutions ≠ IBM(모회사). 각각 별도 careers 항목.
- 국가별 법인: 삼성전자 ≠ Samsung Electronics America. 별도 항목.
- "X subsidiary Y" 형태 (예: "IBM subsidiary SQT"): 자회사 Y가 명시되어 있으면 모회사가 아니라 Y로 별도 항목.

이유: 채용 매칭은 자회사·국가 단위 경험을 구분합니다 (글로벌 vs 로컬, 모회사 vs 자회사 경험). 한 번 통합으로 잃은 정보는 복구 불가능합니다.

통합 대상은 다음만:
- 같은 법인의 같은 직무가 다른 source_section에 다른 표기(국·영문)로 추출되어 중복된 경우.
- 한 법인 안에서 직무 변경(승진, 부서 이동)이 별도 항목으로 추출된 경우 → 한 항목으로 합치고 직무 history는 duties에 기록.

통합할 때는 한 회사의 전체 기간과 세부 직무 기간이 함께 있으면 전체 기간을 최종 값으로 사용하세요.

### raw careers 보존 원칙

raw careers의 각 항목은 본인이 이력서에 명시적으로 적은 정보입니다. 임의로 삭제·축소·합산하지 마세요.

- 본인이 모회사 통합 라인(예: "IBM 2006-2017")과 자회사 세부 라인(IBM Singapore 2014-2017, IBM Korea 2006-2008 등)을 모두 적었다면 → **모두 별도 항목으로 보존**.
- 모회사 통합 라인이 자회사들의 합산처럼 보여도, 본인이 명시한 정보이므로 별도 항목으로 유지하세요. 검수자가 자회사 단위 경험과 그룹 통합 기간을 모두 볼 수 있어야 합니다.
- 통합·중복 제거의 대상은 같은 source_section에서 진짜 입력 오류로 두 번 들어간 경우만입니다.
...
- COMPANY_DUPLICATE: 같은 법인의 같은 직무가 입력 오류로 두 번 추출되어 통합한 경우 (자회사·국가별 법인은 별도 항목이므로 통합 대상 아님)
...
다음 단서가 이력서에 명시되어 있으면 정상 학사 경로로 추정하고 보고하지 마세요:
- "편입", "편입학", "transfer", "Top-up", "탑업" 등 키워드가 학력 라인이나 부가 설명에 명시
- 같은 분야 또는 인접 분야의 다른 학교가 함께 기재 (예: Diploma + Bachelor)
- 직전 학력이 Diploma/HND/Foundation/전문대학 졸업이고 다음 학력이 Bachelor
  (영국·호주·말레이시아 등의 Top-up degree 경로 — 1년 만에 Bachelor 취득이 정상)

보고 대상은 위 어느 것으로도 설명되지 않으면서
정규 학위를 통상 수업연한보다 현저히 짧은 기간에 취득한 경우입니다.
참고 수업연한: 학사 4년, 석사 2년, 박사 3~5년 (단, 학점은행제·야간·
계절학기·편입·Top-up 등으로 단축 가능하므로 "현저히 짧은" 경우만 해당).
