...
3. summary는 이력서에 요약이 명시되어 있으면 그 내용을 바탕으로, 없으면 경력과 학력을 바탕으로 한국어 1~2문장으로 작성하세요. 영문 요약도 한국어로 번역합니다. null로 두지 마세요.
4. core_competencies는 영문이면 한국어로 번역하세요 (예: "Inventory planning" → "재고 관리", "Purchasing" → "구매 관리"). 단, 고유명사(SAP, ERP 등)는 원문 유지.
5. JSON만 출력하세요.
"""
...
end_date와 is_current는 하나의 사실(재직 여부)을 두 필드로 표현한 것입니다.
두 필드가 모순되면 후속 시스템(기간 계산, 중복 탐지)이 오작동합니다.

원칙: end_date가 있으면 퇴사한 것이고, 없으면 재직 중인 것입니다.
is_current는 이 판단과 일치해야 합니다.
모순이 발견되면 end_date 유무를 기준으로 is_current를 교정하고,
교정 사실을 date_evidence에 기록하세요.
...
CAREER_OUTPUT_SCHEMA = """\
{
  ...
  "flags": [
    {
      "type": "string (불일치 유형을 나타내는 식별자, 예: DATE_CONFLICT, DURATION_MISMATCH)",
...
본인이 솔직하게 밝힌 학력 사항(중퇴, 수료 등)은 위조가 아닙니다.
