...
| 앱 | 책임 |
|---|---|
| `accounts` | 로그인(아이디·이메일 + 비밀번호), 조직·멤버십·초대코드, Gmail·텔레그램 연결, 알림 설정 |
...
- **Codex CLI subprocess** (`common/llm.py` default provider): 모든 신규 LLM 호출의 기본. 호스트 `codex exec` 인증 재사용. Claude CLI/Kimi/MiniMax/OpenRouter 등 대체 provider는 `LLM_PROVIDER` 환경변수로 전환.
- **Claude CLI subprocess** (`common/claude.py`, `LLM_PROVIDER=claude_cli`): 기존 호환 provider로 유지.
- **Gemini API**: `data_extraction` 앱의 이력서 배치 추출·정규화 전용. 보이스 인텐트/미팅 분석의 기존 Gemini 경로는 각각 `VOICE_INTENT_PROVIDER=gemini`, `VOICE_MEETING_ANALYSIS_PROVIDER=gemini`일 때만 사용. 다른 용도로 쓰려면 먼저 사용자와 논의.
