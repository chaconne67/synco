...
AI 자동 보정: 오탈자, 서식 통일, 영문명 생성, 회사 소개 자동 작성, 자격증 명칭 매칭.

### 2단계: 상담 내용 입력
직접 입력 또는 녹음 파일 업로드 → Whisper 딕테이션 → AI 정리.

### 3단계: AI 최종 정리 + 검토
초안 + 상담 내용 병합 → 완성본 미리보기. 컨설턴트 직접 수정 가능.

### 4단계: 제출용 변환
포맷(Word/PDF), 언어(국문/국영문/영문), 마스킹 선택 후 변환.
변환 후 output_file에 저장, Submission의 document_file에도 복사.
...
- `projects/templates/projects/partials/draft_*.html` — 각 단계 템플릿
- migration 파일
- 테스트 파일
