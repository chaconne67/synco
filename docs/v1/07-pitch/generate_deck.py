...
add_rect(
    ...
    Inches(11.5),
    Inches(1.0),
    fill_color=RGBColor(0x15, 0x2E, 0x1E),
    border=False,
)
add_text_box(
    s,
...
traction = [
    (
        "2개 GA 계약 체결",
        "공동창업자가 현업 영업자로서 직접 파트너십 체결. Day 1 실행 가능.",
        ACCENT,
    ),
    (
        "CEO DB 3,000~4,000명",
        "공동창업자 본인 DB로 즉시 프로토타입 테스트 가능.",
        ACCENT,
    ),
...
market_data = [
    (
        "TAM",
        "6,000억 ~ 4.8조원",
        "도달 가능 Unique CEO 50만~100만명\nCEO 1인당 연간 120만~480만원 (구독+크레딧+딜 중개)",
        Inches(3.8),
    ),
    (
        "SAM",
...
for col, (header, w) in enumerate(zip(headers, header_widths)):
    add_rect(
        s, x_pos, Inches(1.3), w, Inches(0.5), fill_color=RGBColor(0x25, 0x35, 0x55)
    )
    add_text_box(
        s,
        x_pos + Inches(0.1),
...
for row, (name, match, close, limit, diff) in enumerate(competitors):
    ...
    for col, (text, w) in enumerate(
        zip([name, match, close, limit, diff], header_widths)
    ):
...
why_now = [
    (
        "AI CRM 폭발적 성장",
        "글로벌 AI CRM 시장 CAGR 36%\n14.9조 → 69.8조원\n2025~2027년이 초기→주류 전환 결정적 시기",
        "📈",
        ACCENT,
    ),
    (
        "GA의 구조적 변화",
        "GA협회 2035년 '금융판매전문회사' 비전 공식 채택\n보험 판매 → 종합 금융/비즈니스 연결\nsynco는 이 비전에 정확히 정렬",
        "🏢",
        ACCENT2,
    ),
    (
        "리멤버 매칭 시장 진입",
        "250만 회원의 리멤버가 2026.3 매칭 런칭\n시장을 교육해주지만 = 콜드 매칭\nsynco = 웜 매칭. 같은 시장, 다른 접근",
        "🔥",
        YELLOW,
    ),
]
