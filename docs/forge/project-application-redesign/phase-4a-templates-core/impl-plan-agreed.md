...
- `/projects/<id>/` → 상단 요약 + 탭 구조 (Applications 탭 활성, Timeline/JD 탭 placeholder) + Application 목록 + 각 Application의 ActionItem 패널
- 8개 핵심 템플릿 작성 완료
- Tailwind 빌드 OK, Pretendard 폰트 적용
- 빈 상태(empty state) UI 처리
- 모든 내비게이션 링크에 HTMX progressive enhancement 적용
...
- `projects/project_detail.html` (덮어쓰기, 탭 shell 유지)

### 3.3 Phase 4b로 이월
- 모든 모달 partial (action_complete_modal, action_skip_modal, action_reschedule_modal 등)
...
### T4a.1 — 기존 디자인 스타일 파악
**파일**: 별도 파일 없음
**작업**: 기존 템플릿(`templates/common/base.html`, `common/base_partial.html`, 기존 projects 템플릿 등)에서 사용 중인 스타일을 파악하고, 새 템플릿에서 동일한 스타일을 따른다.

핵심 확인사항:
- 템플릿 상속: `{% extends request.htmx|yesno:"common/base_partial.html,common/base.html" %}` 패턴 사용
- Dashboard는 항상 전체 페이지: `{% extends "common/base.html" %}`
- HTMX 네비게이션: `hx-get + hx-target="#main-content" + hx-push-url="true"`
- `#modal-container`는 `common/base.html`에 이미 존재 — 자식 템플릿에서 중복 선언 금지
- Pretendard 폰트와 Tailwind가 base 템플릿에서 로드됨

---

### T4a.2 — `dashboard.html` (대시보드 메인)
**파일**: `projects/templates/projects/dashboard.html` (기존 파일 덮어쓰기)
**구조**:
```html
  ...
  <div class="flex items-start justify-between gap-4">
      ...
      {% if action.application %}
        <p class="mt-1 text-xs text-slate-400">
          <a href="{% url 'projects:project_detail' action.application.project.id %}"
             hx-get="{% url 'projects:project_detail' action.application.project.id %}"
    ...
    {% if action.status == 'pending' %}
      <div class="flex flex-col gap-2 shrink-0">
        ...
        <button hx-get="{% url 'projects:action_skip' action.id %}"
                hx-target="#modal-container"
                hx-swap="innerHTML"
                class="rounded-md bg-slate-700 px-3 py-1 text-xs text-slate-200 hover:bg-slate-600">
          건너뛰기
        </button>
        <button hx-get="{% url 'projects:action_reschedule' action.id %}"
                hx-target="#modal-container"
                hx-swap="innerHTML"
                class="rounded-md bg-slate-700 px-3 py-1 text-xs text-slate-200 hover:bg-slate-600">
          나중에
...
**변경사항 (원본 대비):**
- 완료/건너뛰기: `hx-post` → `hx-get` + `hx-target="#modal-container"` (모달 오픈)
- 나중에(reschedule) 버튼 추가
- 프로젝트 링크에 HTMX progressive enhancement 추가

---

### T4a.5 — `project_list.html` (2-phase 칸반)
**파일**: `projects/templates/projects/project_list.html` (덮어쓰기)
**구조**:
```html
  ...
  <header class="flex items-center justify-between border-b border-slate-800 px-8 py-6">
    ...
    <div class="flex items-center gap-2">
      <a href="{% url 'projects:project_create' %}"
         hx-get="{% url 'projects:project_create' %}"
         hx-target="#main-content" hx-push-url="true"
...
**변경사항 (원본 대비):**
- `extends "base.html"` → `extends request.htmx|yesno:"common/base_partial.html,common/base.html"`
- `+ 새 프로젝트` 링크에 HTMX progressive enhancement 추가
...
**변경사항 (원본 대비):**
- HTMX progressive enhancement 추가 (`hx-get + hx-target="#main-content" + hx-push-url="true"`)

---

### T4a.8 — `project_detail.html` (프로젝트 상세 메인)
**파일**: `projects/templates/projects/project_detail.html` (덮어쓰기, 탭 구조 유지)
**구조**:
```html
{% extends request.htmx|yesno:"common/base_partial.html,common/base.html" %}
...
<div class="min-h-screen bg-slate-950 text-slate-100">
    ...
    <div class="flex items-start justify-between gap-4">
      ...
      <div class="flex flex-col gap-2 items-end">
        ...
        <a href="{% url 'projects:project_update' project.id %}"
           hx-get="{% url 'projects:project_update' project.id %}"
           hx-target="#main-content" hx-push-url="true"
           class="text-xs text-slate-400 hover:text-slate-200">JD 편집</a>
        {% if project.status == "open" %}
          <button hx-get="{% url 'projects:project_close' project.id %}"
                  hx-target="#modal-container"
                  hx-swap="innerHTML"
  ...
  {# 탭 구조 유지 — Applications 탭만 활성, 나머지 placeholder #}
  <nav class="border-b border-slate-800 px-8">
    <div class="flex gap-6">
      <button class="py-3 text-sm font-medium text-teal-400 border-b-2 border-teal-400"
              data-tab="applications">
        후보자 ({{ applications|length }})
      </button>
      <button class="py-3 text-sm font-medium text-slate-500 hover:text-slate-300"
              data-tab="timeline" disabled>
        타임라인
      </button>
      <button class="py-3 text-sm font-medium text-slate-500 hover:text-slate-300"
              data-tab="jd" disabled>
        JD / 메타
      </button>
    </div>
  </nav>

  <main class="px-8 py-6" id="tab-content">
    <div id="project-applications-container">
  ...
  {# modal-container는 common/base.html에 이미 존재 — 여기서 중복 선언하지 않음 #}
</div>
{% endblock %}
```

**변경사항 (원본 대비):**
- `extends "base.html"` → `extends request.htmx|yesno:...` 패턴
- `project_edit` → `project_update` (올바른 URL name)
- `프로젝트 종료` `hx-get="#"` → `hx-get="{% url 'projects:project_close' ... %}"` + `hx-target="#modal-container"`
- 탭 구조 추가 (Applications 활성, Timeline/JD는 disabled placeholder)
- `#project-applications-container` 래퍼 추가 (HX-Trigger 갱신 대상)
- `<div id="modal-container">` 제거 (base.html에 이미 존재)
- JD 편집 링크에 HTMX progressive enhancement 추가
...
**변경사항 (원본 대비):**
- 3상태 분기: active / dropped / hired (hired_at 처리 추가)
- hired 상태: 읽기 전용 배지, 액션 버튼 숨김
- 드롭 버튼: `hx-get="#"` → `hx-get="{% url 'projects:application_drop' ... %}"` + `hx-target="#modal-container"`
- pending 액션 빈 상태 메시지 추가
- `application.action_items.all` → `application.pending_actions` (뷰에서 미리 필터링하여 전달)
- 후보자 링크에 HTMX progressive enhancement 추가
...
- `/projects/<id>/` → 상단 요약 + 탭 구조 + "아직 매칭된 후보자가 없습니다"
- 관리자에서 Application 1건 생성 후 상세 페이지에 카드 표시 확인
- 완료/건너뛰기/나중에 버튼 클릭 → 모달 컨테이너에 모달 렌더링 확인 (모달 자체는 Phase 4b)

---

## 5. HX-Trigger 갱신 계약

Phase 4a에서는 모달 구현은 Phase 4b로 미루지만, **갱신 경계(refresh boundary)는 이 Phase에서 확정**한다.

| HX-Trigger 이벤트 | 갱신 대상 컨테이너 | 갱신 방법 |
|---|---|---|
| `actionChanged` | `#project-applications-container` | `hx-get="{% url 'projects:project_detail' project.id %}" hx-trigger="actionChanged from:body"` (Applications 섹션 재렌더) |
| `applicationChanged` | `#project-applications-container` | 동일 |
| `actionChanged` | 대시보드 각 섹션 | `hx-get="/dashboard/partial/todo/?scope=overdue"` 등 |

**구현 방식**: 컨테이너에 `hx-get` + `hx-trigger="{event} from:body"` + `hx-swap="innerHTML"` 속성을 선언하여, 모달 POST 응답의 HX-Trigger 헤더에 의해 자동 갱신되도록 한다.
...
`prefetch_related("action_items__action_type")`만으로는 부족하다. 다음 전략을 적용한다:

1. **`current_state` 호출 방지**: 뷰에서 각 Application의 `current_state`를 미리 계산하여 context로 전달. 또는 `Prefetch("action_items", queryset=ActionItem.objects.filter(status="done").order_by("-completed_at")[:1])` 사용
2. **pending_actions 분리**: 뷰에서 `application.pending_actions`를 미리 필터링하여 전달. 템플릿에서 `action_items.all`을 직접 접근하지 않음
3. **ActionItem 카드 관계 prefetch**: `select_related("application__project__client", "application__candidate")` 또는 대시보드 쿼리에서 미리 join

**대시보드 뷰**: `ActionItem.objects.filter(...).select_related("application__project__client", "application__candidate", "action_type")`
**프로젝트 상세 뷰**: `Application.objects.filter(project=project).select_related("candidate").prefetch_related(Prefetch("action_items", queryset=ActionItem.objects.filter(status="pending").select_related("action_type")))`

---

## 7. 검증 체크리스트

- [ ] `dashboard.html` 렌더링 + 빈 상태 메시지
- [ ] `dashboard_todo_list.html` partial 동작
- [ ] `project_list.html` 3컬럼 칸반 표시
- [ ] `kanban_column.html`, `project_card.html` partial 정상
- [ ] `project_detail.html` 상단 요약 + 탭 구조 (Applications 활성, Timeline/JD disabled)
- [ ] `application_card.html` 3상태 표시 (active / dropped / hired)
- [ ] `action_item_card.html` 완료/건너뛰기/나중에 버튼 → 모달 트리거
- [ ] Tailwind 클래스가 빌드되어 시각적으로 적용
- [ ] HTMX 네비게이션: 모든 주요 링크에 `hx-get + hx-target="#main-content" + hx-push-url="true"` 적용
- [ ] #modal-container 중복 없음 (base.html의 것만 사용)
- [ ] HX-Trigger 갱신 계약: 컨테이너에 `hx-trigger` 속성 선언
- [ ] Application 카드 pending 액션 빈 상태 메시지 표시

## 8. 리스크와 대응

| 리스크 | 대응 |
|---|---|
| `application.current_state` N+1 | §6 성능 가이드: 뷰에서 미리 계산 또는 Prefetch with custom queryset |
| ActionItem 카드의 역관계 traversal N+1 | 대시보드: `select_related` join. 상세: `Prefetch` with `select_related` |
...
| 캔들레이트 모델의 필드명(`birth_year`, `current_company`) 실제와 다름 | 확인됨: candidates/models.py에 `birth_year`, `current_company` 필드 존재 |
| 새 템플릿 스타일이 기존 페이지와 불일치 | 기존 템플릿의 스타일을 그대로 따른다 |
| 모달 트리거 버튼이 Phase 4b 모달 없이 동작하지 않음 | 예상 동작: 모달 partial이 없으면 빈 모달 표시. Phase 4b에서 모달 partial 구현 시 자연스럽게 연결 |
...
- Rewrite dashboard.html with today/overdue/upcoming sections
- Rebuild project_list.html as 2-phase + closed kanban
- Rebuild project_detail.html with tab structure + Application list
- Add kanban_column, project_card, application_card, action_item_card partials
- HTMX progressive enhancement on all navigation links
- HX-Trigger refresh contract for actionChanged/applicationChanged
- 3-state application card (active/dropped/hired)
- N+1 prevention via Prefetch + select_related

Refs: FINAL-SPEC.md §5
```

## 10. Phase 4b로 넘기는 인터페이스

- 메인 시각 구조 확립 (기존 디자인 스타일 유지)
- 탭 구조: Applications 활성, Timeline/JD disabled → Phase 4b에서 활성화
- HTMX 모달 트리거 버튼은 `#modal-container`로 swap → Phase 4b가 모달 partial 작성
- HX-Trigger 갱신 계약 확정: `actionChanged` / `applicationChanged` 이벤트
...
<!-- forge:phase-4a-templates-core:impl-plan:complete:2026-04-14T20:05:00Z -->
