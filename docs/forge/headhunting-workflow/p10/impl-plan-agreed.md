...
| # | 이슈 | 적용 내용 |
|---|------|----------|
| I-01 | URL 메서드 계약 | posting_site_add/update는 GET/POST 허용 (인라인 폼 패턴) |
| I-02 | is_active 폼 포함 | PostingSiteForm fields에 is_active 추가 |
| I-03 | UniqueConstraint 500 방지 | posting_site_update에 IntegrityError 처리 추가 |
| I-04 | 소프트삭제 후 재등록 | posting_site_add에서 비활성 레코드 재활성화 로직 |
| I-05 | 데이터 소스 매핑 | 프롬프트에 섹션별 우선순위 규칙 추가 |
| I-06 | Gemini 오류 보존 테스트 | RuntimeError 시 기존 posting_text 보존 검증 |
| I-07 | 덮어쓰기 보호 | posting_generate에 overwrite 파라미터 추가 |
| I-08 | Lint 수정 | 미사용 import/변수 제거 |
| I-09 | 테스트 카운트 | Task 5 기대 개수 수정 |
| I-10 | 파일명 테스트 | regex 패턴 매칭으로 강화 |
    ...
    class Meta:
        ...
        widgets = {
            ...
            "is_active": forms.CheckboxInput(
                attrs={"class": "rounded border-gray-300 text-primary focus:ring-primary"}
            ),
...
```python
from .models import Contact, Interview, JDSource, Offer, PostingSite, PostingSiteChoice, Project, Submission
```
...
class TestPostingGenerateView:
    ...
    def test_generate_runtime_error_preserves_existing(self, auth_client, project, monkeypatch):
...
git commit -m "feat(p10): add posting generate, edit, download views with overwrite protection"
```
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if existing:
                ...
                for field in ["posted_at", "applicant_count", "url", "notes", "is_active"]:
                    setattr(existing, field, form.cleaned_data.get(field, getattr(existing, field)))
...
<!-- forge:p10:구현담금질:complete:2026-04-08T23:55:00+09:00 -->
