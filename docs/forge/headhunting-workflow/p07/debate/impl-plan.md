...
### Submission (projects 앱 — 기존 모델에 필드 추가/수정)

P01에서 기본 모델 정의됨. 추가/수정 필요 필드:
...
후보자 드롭다운 (컨택 결과 "관심"인 후보자만), 양식 선택 (4가지), 서류 파일 업로드 (Word/PDF, 최대 10MB), 메모.
P06 컨택 탭 "추천 서류 작성 →" 클릭 시 후보자가 미리 선택된 상태로 진입.
...
"제출하기 →": draft → submitted 전환 + submitted_at 기록
"면접 등록 →": 통과 건에서 Interview 생성 연결 (P09에서 구현)
...
결과: 통과/탈락, 피드백 내용, client_feedback + client_feedback_at 저장
...
- `projects/models.py` — Submission 모델 필드 추가/수정
