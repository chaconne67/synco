(async function () {
  const { parserSaramin } = await chrome.storage.sync.get({ parserSaramin: true });
  if (!parserSaramin) return;

  const SaraminParser = {
    parse() {
      const getText = (sel) => document.querySelector(sel)?.textContent?.trim() || "";

      const name = getText(".resume-name") || getText(".applicant-name");
      const position = getText(".resume-position") || getText(".desired-position");
      const company = getText(".current-company") || getText(".recent-company");

      const careers = [];
      document.querySelectorAll(".career-row, .experience-row").forEach((item) => {
        careers.push({
          company: item.querySelector(".company-name, .corp-name")?.textContent?.trim() || "",
          position: item.querySelector(".position-name, .job-title")?.textContent?.trim() || "",
          department: item.querySelector(".dept-name")?.textContent?.trim() || "",
          start_date: item.querySelector(".start-date, .period-start")?.textContent?.trim() || "",
          end_date: item.querySelector(".end-date, .period-end")?.textContent?.trim() || "",
          is_current: "false",
          duties: item.querySelector(".job-description")?.textContent?.trim() || "",
        });
      });

      const educations = [];
      document.querySelectorAll(".education-row, .edu-row").forEach((item) => {
        const years = (item.querySelector(".period, .edu-period")?.textContent || "").match(/\d{4}/g) || [];
        educations.push({
          institution: item.querySelector(".school-name, .univ-name")?.textContent?.trim() || "",
          degree: item.querySelector(".degree-name")?.textContent?.trim() || "",
          major: item.querySelector(".major-name")?.textContent?.trim() || "",
          start_year: years[0] || "",
          end_year: years[1] || "",
        });
      });
      ...
      return {
        ...
        source_site: "saramin", source_url: location.href,
  ...
  async function init() {
    ...
    const profile = SaraminParser.parse();
